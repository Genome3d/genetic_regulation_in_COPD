...
pacman::p_load(ggplot2, pander, dplyr, tidyr, ggrepel, tidyverse, reshape2, RColorBrewer,
               ...
               vroom, ggforce, cowplot, pheatmap, readxl, disgenet2r, httr)
...
### 1.1. [UPDATE] Percentage of spatial lung-specific eQTL SNPs and non-eQTL SNPs among all variants (genotyped from lung samples obtained from GTEx v8).
...
### 1.2. [UPDATE] Percentage of genes spatially regulated in the lung among all genes from GTEx v8.
...
### 3.1. [UPDATE] Number of human protein-protein interactions (PPI) in the STRING PPI network and lung-specific PPI network.
...
# # cluster 1 - TGF-beta signaling pathway
...
# #write.table(c_1.df, file = "results/kegg/copd_LSPPIN_cluster1_kegg_fdr_known.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
# 
# # cluster 2 - Regulation of actin cytoskeleton, Focal adhesion 
...
# #write.table(c_2.df, file = "results/kegg/copd_LSPPIN_cluster2_kegg_fdr_known.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
# 
# # cluster 3 - Neuroactive ligand-receptor interaction, Cholinergic synapse  
...
# #write.table(c_3.df, file = "results/kegg/copd_LSPPIN_cluster3_kegg_fdr_known.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
# 
# # cluster 4 - Unknown 
...
# 
# # cluster 5 Immune-related pathways
...
# #write.table(c_5.df, file = "results/kegg/copd_LSPPIN_cluster5_kegg_fdr_known.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
# 
# # cluster 6 - Sulfur relay system, Folate biosynthesis
...
# #write.table(c_6.df, file = "results/kegg/copd_LSPPIN_cluster6_kegg_fdr_known.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
# 
# # cluster 7 - Insulin resistance, Insulin signaling pathway
...
# #write.table(c_7.df, file = "results/kegg/copd_LSPPIN_cluster7_kegg_fdr_known.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
# 
# # cluster 8 - Phagosome
# communities(copd_lc)[8] # to get gene names
# c_8 <- communities(copd_lc)$'8' # to get gene names
# c_8_path <- query_kegg(c_8)
# c_8_paths <- c_8_path$result
# c_8.df <- apply(c_8_paths, 2, as.character)
# #write.table(c_8.df, file = "results/kegg/copd_LSPPIN_cluster8_kegg_fdr_known.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
# 
# # cluster 9 - Axon guidance, Regulation of actin cytoskeleton 
...
# #write.table(c_9.df, file = "results/kegg/copd_LSPPIN_cluster9_kegg_fdr_known.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
```

### 4.3. Gene Ontology (GO) enrichment analysis of the COPD-associated genes within the LSPPIN clusters.
...
```{r disgenet_query, fig.width=10, fig.height=7}
auth_params <- list("email"="e.golovina@auckland.ac.nz","password"="2022Genes!13")
